...
class MCTformerV2(VisionTransformer):
    def __init__(self, *args, **kwargs):
        ...
        self.head_aug = nn.Conv2d(self.embed_dim, self.num_classes, kernel_size=3, stride=1, padding=1)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.head.apply(self._init_weights)
        self.head_aug.apply(self._init_weights)
    ...
    def forward(self, x, return_att=False, n_layers=12, attention_type='fused', strong_aug=False):
        w, h = x.shape[2:]
        x_cls, x_patch, attn_weights = self.forward_features(x) #x_cls is (B, C, D), x_patch is (B, N, D)
        x_img = x_patch.clone()
        n, p, c = x_patch.shape
        #print(p)
        if w != h:
        ...
        n, _, h, w = x_patch.shape
        c = self.num_classes

        attn_weights = torch.stack(attn_weights)  # 12 * B * H * N * N
        attn_weights = torch.mean(attn_weights, dim=2)  # 12 * B * N * N

        mtatt = attn_weights[-n_layers:].sum(0)[:, 0:self.num_classes, self.num_classes:] # (n,c, h*w)

        mtatt = mtatt.reshape([n, c, h, w])

        if strong_aug:
            x_patch = self.head_aug(x_patch)
        else:
            x_patch = self.head(x_patch)  # (B,C,H,W)
...
class MCTformerV1(VisionTransformer):
    ...
    def forward(self, x, n_layers=12, return_att=False):
        x_cls, x_img,attn_weights = self.forward_features(x)
        print(x_img.shape)
        attn_weights = torch.stack(attn_weights)  # 12 * B * H * N * N
        attn_weights = torch.mean(attn_weights, dim=2)  # 12 * B * N * N
        mtatt = attn_weights[-n_layers:].sum(0)[:, 0:self.num_classes, self.num_classes:]
        patch_attn = attn_weights[:, :, self.num_classes:, self.num_classes:]
        #x_cls_logits = torch.matmul(x_cls, x_img.permute(0, 2,1)) # b c m
        #x_cls_logits = x_cls_logits.mean(-1)
